...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230113000947_addingad')
BEGIN
    DECLARE @var4 sysname;
    SELECT @var4 = [d].[name]
    FROM [sys].[default_constraints] [d]
    INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
    WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Calculations]') AND [c].[name] = N'Plan');
    IF @var4 IS NOT NULL EXEC(N'ALTER TABLE [Calculations] DROP CONSTRAINT [' + @var4 + '];');
    ALTER TABLE [Calculations] ALTER COLUMN [Plan] datetime2 NOT NULL;
    ALTER TABLE [Calculations] ADD DEFAULT '0001-01-01T00:00:00.0000000' FOR [Plan];
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230113000947_addingad', N'6.0.9');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230113001416_addingad2')
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230113001416_addingad2', N'6.0.9');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230113115205_addingVas')
BEGIN
    ALTER TABLE [Sales] ADD [SD_SUBTYPE] nvarchar(max) NULL;
END;
GO
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230113115205_addingVas', N'6.0.9');
END;
GO
...
IF NOT EXISTS(SELECT * FROM [__EFMigrationsHistory] WHERE [MigrationId] = N'20230113115356_addingVas2')
BEGIN
    CREATE TABLE [Vas] (
        [Id] int NOT NULL IDENTITY,
        [DAY] datetime2 NOT NULL,
        [MONTH] nvarchar(max) NULL,
        [EQUIPID] nvarchar(max) NULL,
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20230113115356_addingVas2', N'6.0.9');
END;
GO
